...
def test_depthmaps_step_1(mocked_file_check, mocked_cv2, mocked_depth_map_func):
    '''
    ...
    test_converter.convert_video("any_input", "any_output", False, 1, True)
    assert mocked_writer.write.call_count == total_frames_count-1

@mock.patch('Integration.Video_To_Depthmap.video_converter.create_depth_map')
@mock.patch('Integration.Video_To_Depthmap.video_converter.cv2')
@mock.patch('Integration.Video_To_Depthmap.video_converter.check_file_exists')
def test_depthmaps_step_3(mocked_file_check, mocked_cv2, mocked_depth_map_func):
    '''
    ...
    test_converter.convert_video("any_input", "any_output", False, 3, True)
...
def test_depthmaps_step_0(mocked_file_check):
    ...
    with pytest.raises(ValueError):
        test_converter.convert_video("any_input", "any_output", False, 0, True)
...
@mock.patch('Integration.Video_To_Depthmap.video_converter.cv2')
@mock.patch('Integration.Video_To_Depthmap.video_converter.check_file_exists')
def test_depthmaps_large_step(mocked_file_check, mocked_cv2, mocked_depth_map_func):
    '''
    ...
    test_converter.convert_video("any_input", "any_output", False, total_frames_count+1, True)
    assert mocked_writer.write.call_count == 0

@mock.patch('Integration.Video_To_Depthmap.video_converter.create_depth_map')
@mock.patch('Integration.Video_To_Depthmap.video_converter.cv2')
@mock.patch('Integration.Video_To_Depthmap.video_converter.check_file_exists')
def test_one_frame(mocked_file_check, mocked_cv2, mocked_depth_map_func):
    # set required mocks
...
def test_performance():
    ...
    print "Timing test:"
    tester.startTimer()
    test_converter.convert_video("../Video to image/test.mp4", "out.mp4", True, 1, False)
    tester.stopTimer()
