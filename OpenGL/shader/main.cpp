...
const char *WINDOWTITLE = { "Assignment 5: uncertainty" };
const char *GLUITITLE   = { "Spiral" };
...
Display( )
{
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char*)"Project 5" );
...
InitMenus( )
{
	...
	int distmenu = glutCreateMenu( DoDistortMenu );
	glutAddMenuEntry( "On",  1 );
	glutAddMenuEntry( "Off",  0 );

	int patmenu = glutCreateMenu( DoPatMenu );
	glutAddMenuEntry( "On",  1 );
	glutAddMenuEntry( "Off",  0 );

	int mainmenu = glutCreateMenu( DoMainMenu );
	glutAddSubMenu(   "Axes",          axesmenu);
	glutAddSubMenu(   "Distort Shape", distmenu);
	glutAddSubMenu(   "Animate Pattern", patmenu);
